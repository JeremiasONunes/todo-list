...
// Ícones de linha (`lucide-react`) — mesma troca de emoji-como-ícone já
// feita em `Sidebar`/`LandingFeatures`/`EmptyState`/`StatCard`. `Tarefas` e
// `Analytics` reusam de propósito os mesmos ícones do `Sidebar` (não
// escolhas novas) — é o mesmo conceito em dois lugares do app, faz sentido
// ser o mesmo símbolo nos dois.
const CONCEITOS = [
  {
    icon: Component,
...
/**
 * Componente Sobre — página informativa sobre a aplicação. `Header` e o
 * wrapper `<div min-h-screen>`/`<main>` saíram daqui na Fase 2 (agora
 * resolvidos por `PublicLayout`).
 *
 * Fase 8: migrada de classes Tailwind cruas (`bg-gradient-to-r`,
 * `glass-effect`...) pra CSS Modules + `Card` — era a última página do app
 * ainda nesse estilo antigo, uma inconsistência real que "refinamento" desta
 * fase existe pra fechar. O texto também foi atualizado: a versão anterior
 * descrevia só a lista de tarefas original (Fase 1-2), sem mencionar
 * autenticação, painel ou analytics (Fases 4-7).
 */
function Sobre() {
  return (
